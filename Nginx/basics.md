...
Contex {}, stores directives

### Basics

`worker processes` - should equal cpu cores amount

`worker_connections` - how many connections each worker processes, default 512, increases memory usage, cannot exceed open files limit (linux)
...
Contexts - top level directives:
- events - general connection processing
- http - HTTP trafic
- mail - mail trafiic
- stream - TCP and UDP traffic

Directives pleaced outside the contexts are called main contect directives.
...
Send content type to a client:

```nginx
include mime.types; # inside http context (block)
```

#### Modifying proxied page

Forwarding host, real client IP, custom headers
...
```
`upstream servers` - traffic going from a client toward the source or higher level infra, in this case the application server.

...
* `frame-src`, `child-src` - Web Workers API origins, `<iframe>` and `<frame>` sources
...
* `frame-ancestors` - it's the equivalent of `X-Frame-Options` header. Defines which origins can load web-app inside `<iframe>` tag. `none` === `DENY`, `self` === `SAMEORIGIN`. In case of nested iframes, all origins must be whitelisted. 
